...
1. Lire `domain/model/valueobject/Money.java` et `domain/model/valueobject/Email.java` comme modèles
2. Créer la classe dans `domain/model/valueobject/` (ou `domain/model/valueobject/id/` si c'est un identifiant, modèle `OrderId.java`)
3. Valider tous les arguments dans le constructeur ; aucun setter
4. Créer `XTest.java` dans `src/test/.../domain/model/valueobject/` avec au minimum : cas nominal, null refusé, valeur invalide refusée
